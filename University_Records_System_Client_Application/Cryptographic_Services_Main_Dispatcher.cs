...
namespace University_Records_System_Client_Application
{
    ...
    {
        protected static async Task<object> Dispatcher(Cryptographic_Serives_Dispatcher_Controller.Option option, object first_setter, object second_setter, object third_setter)
        {
            object result = new object();

            switch (option)
            {
                case Cryptographic_Serives_Dispatcher_Controller.Option.Content_Hasher:
                    result = (first_setter != null && first_setter.GetType() == typeof(string)) ? await Content_Hasher(first_setter as string) : result;
                    break;

                case Cryptographic_Serives_Dispatcher_Controller.Option.Load_X509_Certificate_Into_Store:
                ...
                case Cryptographic_Serives_Dispatcher_Controller.Option.Save_Log_In_Key:
                ...
                case Cryptographic_Serives_Dispatcher_Controller.Option.Delete_Log_In_Sesion_Key:
                    result = await Delete_Log_In_Sesion_Key();
                    break;

                case Cryptographic_Serives_Dispatcher_Controller.Option.Load_Log_In_Session_Key:
